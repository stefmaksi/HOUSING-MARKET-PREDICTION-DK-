...
def no_null_house_id(db_path: str) -> int:
    conn = sqlite3.connect(db_path)
    r = pd.read_sql_query(
        "SELECT COUNT(*) as n FROM fact_sales WHERE house_id IS NULL OR TRIM(house_id) = ''",
        conn,
    )
    conn.close()
...
def numeric_issues(db_path: str) -> dict:
    ...
    r = pd.read_sql_query(
        "SELECT COUNT(*) as n FROM fact_sales WHERE purchase_price IS NULL OR purchase_price <= 0",
        conn,
    )
    issues["bad_purchase_price"] = int(r["n"].iloc[0])
    # sqm_price should be >= 0
    r = pd.read_sql_query(
        "SELECT COUNT(*) as n FROM fact_sales WHERE sqm_price IS NULL OR sqm_price < 0",
        conn,
    )
...
def run_all(db_path: str) -> None:
    ...
    assert (
        null_houses == 0
    ), f"Found {null_houses} rows in fact_sales with NULL or empty house_id"
    issues = numeric_issues(db_path)
    assert (
        issues["bad_purchase_price"] == 0
    ), f"Found {issues['bad_purchase_price']} invalid purchase_price values"
    assert (
        issues["bad_sqm_price"] == 0
    ), f"Found {issues['bad_sqm_price']} invalid sqm_price values"
    return None
