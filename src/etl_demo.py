...
def show_counts_and_preview(db_path):
    if not os.path.exists(db_path):
        raise FileNotFoundError(f"DB file not found: {db_path}. Run src/etl.py first or set the correct db_path.")
...
LIMIT 10""", con)
    con.close()
    ax = sales_by_region.plot.bar(x='region', y='sales_count', legend=False, figsize=(10,4))
...
def plot_price_distribution(db_path):
    if not os.path.exists(db_path):
        raise FileNotFoundError(f"DB file not found: {db_path}. Run src/etl.py first or set the correct db_path.")
    con = sqlite3.connect(db_path)
    prices = pd.read_sql_query('SELECT purchase_price FROM fact_sales WHERE purchase_price IS NOT NULL', con)
    con.close()
