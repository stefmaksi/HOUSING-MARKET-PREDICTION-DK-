...
def transform(db_path: str):
    ...
    try:
        ...
        print(
            "ERROR: Could not read 'staging' table. Run src/etl.py first.",
            file=sys.stderr,
        )
        conn.close()
        sys.exit(1)
    ...
    dim_property = (
        staging[prop_present].drop_duplicates(subset=["house_id"])
        if "house_id" in prop_present
        else pd.DataFrame()
    )
    ...
    if "%_change_between_offer_and_purchase" in fact.columns:
        fact = fact.rename(
            columns={"%_change_between_offer_and_purchase": "pct_change"}
        )
